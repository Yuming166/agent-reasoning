CREATE OR REPLACE TABLE `ictdata-507912.exgraph.wallet_asof_features_20220901`
PARTITION BY snapshot_date CLUSTER BY target_address AS
WITH snapshots AS (
  SELECT DATE '2022-09-01' AS snapshot_date
),
targets AS (
  SELECT DISTINCT target_address, target_exgraph_node_id
  FROM `ictdata-507912.exgraph.target_event_sequences_20220301_20221001`
  WHERE block_timestamp >= TIMESTAMP('2022-03-01')
    AND block_timestamp <  TIMESTAMP('2022-10-01')
),
events AS (
  SELECT
